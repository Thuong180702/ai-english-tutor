            ...
            <div className={`max-w-md w-full ${theme.cardBg} p-8 rounded-3xl shadow-2xl border ${theme.cardBorder}`}>
                ...
                {authMode === 'forgot' ? (
                    ...
                    <form onSubmit={authMode === 'login' ? handleEmailLogin : handleRegister} className="space-y-4 animate-in fade-in">
                        ...
                        {authMode === 'login' && (<div className="text-right"><button type="button" onClick={() => { setAuthMode('forgot'); setAuthError(""); }} className={`text-xs font-bold ${isDarkMode ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-800'}`}>Quên mật khẩu?</button></div>)}
